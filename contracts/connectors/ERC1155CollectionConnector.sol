...
/**
 * @title ERC1155CollectionConnector
 * @author Souq.Finance
 * @notice The ERC1155 Collection Connector
 * @notice License: https://souq-peripheral-v1.s3.amazonaws.com/LICENSE.md
 */
contract ERC1155CollectionConnector is ICollectionConnector, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    uint256 public version;
    address public addressesRegistry;
    //attributes mapping to convert token ids to a rarity score
    ...
    /// @inheritdoc ICollectionConnector
    function getAttribute(address _add, uint256 _id) external view returns (uint256) {
        return attributes[_add][_id];
    }

    /// @inheritdoc ICollectionConnector
    function setAttribute(address _add, uint256 _id, uint256 _attribute) external onlyAdmin {
        attributes[_add][_id] = _attribute;
    }

    /// @inheritdoc ICollectionConnector
    function setAttributeBatch(address _add, uint256[] calldata _ids, uint256[] calldata _attributes) external onlyAdmin {
        require(_ids.length == _attributes.length, Errors.ARRAY_NOT_SAME_LENGTH);
        for (uint256 i = 0; i < _ids.length; ++i) {
            attributes[_add][_ids[i]] = _attributes[i];
        }
    }
